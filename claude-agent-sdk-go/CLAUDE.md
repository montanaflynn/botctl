...
`claude-agent-sdk-go` is a Go SDK that wraps the Claude CLI (`claude`) for programmatic use. It spawns the CLI as a subprocess, communicates via stream-json over stdin/stdout, and provides Go types for all message types in the protocol. Used by the `botctl` process manager as a local replace in its `go.mod`.
